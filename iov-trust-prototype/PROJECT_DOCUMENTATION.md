...
Here is a guide to the files in `iov-trust-prototype/`:
...
```powershell
cd "c:\A Developer's Stuff\IITP Intern\SIM2\iov-trust-prototype"
...
2.  **Results Folder**: Navigate to `iov-trust-prototype/results/`.
...
3.  Use the logic in `traci_control/run_sumo.py` to replace the random loop in `run_experiment.py`.
